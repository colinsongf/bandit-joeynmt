...
def validate_on_data(model, data, batch_size, use_cuda, max_output_length,
    ...
    if isinstance(model, DeliberationModel):
        ...
        with torch.no_grad():
            ...
            valid_src_attention_scores = []
            valid_d1_attention_scores = []
            total_loss = 0
            total_ntokens = 0
            for valid_i, valid_batch in enumerate(iter(valid_iter), 1):
                ...
                output1, attention_scores, output2, src_attention_scores, \
                d1_attention_scores = model.run_batch(
                    batch=batch, beam_size=beam_size, beam_alpha=beam_alpha,
                    max_output_length=max_output_length)

                # sort outputs back to original order
                all_outputs1.extend(output1[sort_reverse_index])
                all_outputs2.extend(output2[sort_reverse_index])
                valid_attention_scores.extend(
                    attention_scores[sort_reverse_index]
                    if attention_scores is not None else [])
                valid_src_attention_scores.extend(
                    src_attention_scores[sort_reverse_index]
                    if src_attention_scores is not None else [])
                valid_d1_attention_scores.extend(
                    d1_attention_scores[sort_reverse_index]
                    if d1_attention_scores is not None else [])
            ...
            decoded_valid1 = arrays_to_sentences(arrays=all_outputs1,
                                                vocabulary=model.trg_vocab,
                                                cut_at_eos=True)
            decoded_valid2 = arrays_to_sentences(arrays=all_outputs2,
                                                 vocabulary=model.trg_vocab,
                                                 cut_at_eos=True)
            ...
            valid_hypotheses1 = [join_char.join(t) for t in decoded_valid1]
            valid_hypotheses2 = [join_char.join(t) for t in decoded_valid2]
            ...
            if level == "bpe":
                ...
                valid_hypotheses1 = [bpe_postprocess(v) for
                                    v in valid_hypotheses1]
                valid_hypotheses2 = [bpe_postprocess(v) for
                                    v in valid_hypotheses2]
            ...
            if len(valid_references) > 0:
                ...
                if eval_metric.lower() == 'bleu':
                    ...
                    current_valid_score = chrf(valid_hypotheses2,
                                               valid_references)
                    current_aux_valid_score = chrf(valid_hypotheses1,
                                               valid_references)
                elif eval_metric.lower() == 'token_accuracy':
                    current_valid_score = token_accuracy(valid_hypotheses2,
                                                         valid_references,
                                                         level=level)
                    current_aux_valid_score = token_accuracy(valid_hypotheses1,
                                                         valid_references,
                                                         level=level)
                elif eval_metric.lower() == 'sequence_accuracy':
                    current_valid_score = sequence_accuracy(valid_hypotheses2,
                                                            valid_references)
                    current_aux_valid_score = sequence_accuracy(valid_hypotheses1,
                                                            valid_references)
            else:
